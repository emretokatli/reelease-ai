...
export default function ImageToVideoTemplates() {
  ...
  // Filter for image-to-video templates
  const videoTemplates = templates.filter((t: AITemplate) =>
    t.prompt.toLowerCase().includes('animate') ||
    t.title.toLowerCase().includes('image to video') ||
    t.category_id?.name?.toLowerCase().includes('image to video')
  )

  const handleCreateNew = () => {
    router.push(`${ROUTES.IMAGE_TO_VIDEO}/generate`)
  }

  const handleUseTemplate = (templateId: string) => {
    router.push(`${ROUTES.IMAGE_TO_VIDEO}/generate?templateId=${templateId}`)
  }
    ...
    <div className="max-w-6xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="space-y-1">
          <h1 className="text-4xl font-extrabold tracking-tight bg-clip-text text-transparent bg-gradient-to-r from-primary to-purple-400">
            {t('image_to_video', { defaultValue: 'Image to Video' })}
          </h1>
          <p className="text-muted-foreground text-lg">
            {t('image_to_video_desc', { defaultValue: 'Animate your images into stunning videos with AI' })}
          </p>
        </div>
        <Button
        ...
        >
          ...
          {t('create_blank_image_video', { defaultValue: 'Create Image-to-Video' })}
        </Button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <Card
          ...
          onClick={handleCreateNew}
        >
          <CardContent className="h-full flex flex-col items-center justify-center p-4 space-y-4">
            ...
            <div className="text-center">
              ...
              <p className="text-sm text-muted-foreground mt-1 px-4">
                {t('create_video_without_template', { defaultValue: 'Upload your images and watch them animate' })}
              </p>
            </div>
          </CardContent>
        </Card>
